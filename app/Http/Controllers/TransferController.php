...
class TransferController extends Controller
{
	...
	public function create()
	{
		...
		if ($role->hasPermissionTo('transfers-add')) {
			...
			$user = auth()->user();
			if($user->role_id == 1) {
				$lims_warehouse_list_to =	$lims_warehouse_list_from = Warehouse::where('is_active', true)->get();
			}
			else {
				$lims_warehouse_list_from = Warehouse::where('is_active', true)->where('id', $user->warehouse_id)->get();
				$lims_warehouse_list_to = Warehouse::where('is_active', true)->whereNotIn('id', [$user->warehouse_id])->get();
			}

			return view('transfer.create', compact('lims_warehouse_list_from', 'lims_warehouse_list_to'));
		} else
			return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
	}
	...
	public function store(Request $request)
	{
		...
		return redirect("transfers/$lims_transfer_data->id/edit")->with('message', 'Transfer created successfully');
	}
	...
	public function edit($id)
	{
		...
		if ($role->hasPermissionTo('transfers-edit')) {
			...
			$lims_product_transfer_data = ProductTransfer::where('transfer_id', $id)->get();
			return view('transfer.edit', compact('lims_warehouse_list_from', 'lims_warehouse_list_to', 'lims_transfer_data', 'lims_product_transfer_data'));
		} else
			return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
	}
	...
	public function update(Request $request, $id)
	{
		...
		if ($request->status == 1) {
			$received = ProductTransfer::where('transfer_id', $id)->get();
			$this->generateLocationsReceive($received);
			$this->generateLocationsSend($received);
		}
	...
	public function assignLocationsReceive(Request $request)
	{

		$locationId = $request->location_id;

		$productLocationIds = json_decode($request->productLocationIds);

		$productLocations = ProductLocation::whereIn('id', $productLocationIds)->get();

		$lastLocations = [];
		foreach ($productLocations as $productLocation) {
			if (!$productLocation->sales_location_id) {
				// si el producto tiene una ubicacion lo reubicamos
				if ($productLocation->location_id) {

					$productLocation->date_out = now();
					$productLocation->before_location_id = $productLocation->id;
					$productLocation->save();

					$productLocation = ProductLocation::create([
						"product_id" => $productLocation->product_id,
						"product_transfer_id" => $productLocation->product_transfer_id
					]);
				}
				$lastLocations[] = $productLocation->id;
			}
		}

		if (count($lastLocations)) {
			ProductLocation::whereIn('id', $lastLocations)->update(['location_id' => $locationId]);
		}

		return [
			'success' => true,
			'location' => Location::find($locationId),
			'message' => __('file.The products have been located correctly')
		];

	}

	private function generateLocationsReceive($transfers)
	{
		foreach ($transfers as $transfer) {
			for ($i = 0; $i < $transfer->qty; $i++) {
	...
	private function generateLocationsSend($transfers)
	{
		foreach ($transfers as $transfer) {
			for ($i = 0; $i < $transfer->qty; $i++) {
	...
	public function productInWarehouseSend(Request $request)
	{
		$sales_location_id = $request->sales_location_id;
		$sales_warehouse_id = $request->sales_warehouse_id;

		$saleLocation = SalesLocation::find($sales_location_id);

		$data = [];
		ProductLocation::whereNull('product_locations.date_out')
			->whereNotNull('product_locations.location_id')
			->leftjoin('location', 'location.id', '=', 'product_locations.location_id')
			->where('product_locations.product_id', $saleLocation->product_id)
			->where('location.warehouse_id', $sales_warehouse_id)
			->with(['product', 'location'])
			->select('product_locations.*')
			->get()->map(function ($productLocation) use (&$data) {

				if (!isset($data[$productLocation->location->id])) {
					$data[$productLocation->location->id] = [
						'key' => $productLocation->location->key,
						'name' => $productLocation->product->name,
						'code' => $productLocation->product->code,
						'count' => 0
					];
				}
				$data[$productLocation->location->id]['count']++;
	...
	public function assignLocationsSend(Request $request)
	{
		...
		if(!$productLocation = $saleLocation->productLocation){

			$productLocation = ProductLocation::whereNull('product_locations.date_out')
				->whereNotNull('product_locations.location_id')
			...
			$productLocation->date_out = now();
			$productLocation->sales_location_id = $saleLocation->id;
			$productLocation->save();
		}
		...
		return [
			'success' => true,
			'location' => Location::find($locationId),
			'date_out'=> $productLocation->date_out->format('d/m/Y H:i a'),
			'message' => __('file.El producto ha salido del inventario correctamente')
		];
	}
