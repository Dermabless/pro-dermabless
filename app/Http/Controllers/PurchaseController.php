...
class PurchaseController extends Controller
{
        ...
        if ($role->hasPermissionTo('purchases-add')) {
            $lims_supplier_list = Supplier::where('is_active', true)->get();
						...
						if($user->role_id == 1) {
							...
							$lims_warehouse_list= Warehouse::where('is_active', true)->where('id', $user->warehouse_id)->get();
						}
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            $purchases [] = ProductPurchase::create($product_purchase);
        }

        if ($lims_purchase_data->status == 1) {
            $this->generateLocations($purchases);
        }
            ...
            if (!$productLocation->sales_location_id) {
                ...
                if ($productLocation->location_id) {
                    ...
                    $productLocation = ProductLocation::create([
                        ...
                        "product_purchase_id" => $productLocation->product_purchase_id
                    ]);
                }
    ...
    private function generateLocations($purchases)
    {
    ...
    public function update(Request $request, $id)
    {
        ...
        $statusPurhase = $lims_purchase_data->status;
        ...
        if ($lims_purchase_data->status != 1) {
            ...
            foreach ($product_id as $key => $pro_id) {
                ...
                $product_purchase['qty'] = isset($qty[$key]) ? $qty[$key] : 0;
        ...
        if ($request->status == 1 && $statusPurhase != 1) {
            $purchases = ProductPurchase::where('purchase_id', $id)->get();
            $this->generateLocations($purchases);
        }
