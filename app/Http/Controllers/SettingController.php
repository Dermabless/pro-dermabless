...
class SettingController extends Controller
{
	...
	public function backup()
	{
		...
		if (!empty($sqlScript)) {
			$randomKey = (\Illuminate\Support\Str::random(22));
			// Save the SQL script to a backup file
			$backup_file_name = storage_path() . '/app/' . $database_name . '_backup_' . time() . '_' . $randomKey . '.sql';
			...
			$zip = new ZipArchive();
			$zipFileName = $database_name . '_backup_' . time() . '_' . $randomKey . '.zip';
			$zip->open(storage_path() . '/app/' . $zipFileName, ZipArchive::CREATE);
			$zip->addFile($backup_file_name, $database_name . '_backup_' . time() . '_' . $randomKey . '.sql');
			$zip->close();
		}

		return redirect('setting/download?name=' . $zipFileName);
	}

	public function download(Request $request)
	{
		logger("File Download: $request->name");
